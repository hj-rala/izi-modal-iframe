    ...
    iziModal.prototype = {
        ...
        init: function(element, options) {
            ...
            if (options.iframe === true) {
                this.$element.html('<div class="' + PLUGIN_NAME + '-wrap"><div class="' + PLUGIN_NAME + '-content"><iframe class="' + PLUGIN_NAME + '-iframe" iframe scrolling="no"></iframe>' + this.content + "</div></div>");
        ...
        open: function(param) {
          ...
          that.recalcLayout();
				    that.timer = setTimeout(updateTimer, 600);
					//console.log(1);
                })();
                (function urlHash() {
...
//                    }
        ...
        recalcLayout: function() {
            var that = this,
                ...
                contentHeight = this.$element.find('.'+PLUGIN_NAME+'-content').length > 0 ? this.$element.find('.'+PLUGIN_NAME+'-content')[0].scrollHeight : 0,
                outerHeight = contentHeight + this.headerHeight,
                wrapperHeight = this.$element.innerHeight() - this.headerHeight,
                modalMargin = parseInt(-((this.$element.innerHeight() + 1) / 2)) + 'px',
                scrollTop = this.$wrap.scrollTop(),
				frameH =  this.$element.find('.' + PLUGIN_NAME + '-iframe').contents().find('body')[0].scrollHeight;
            ...
            if (this.$element.find('.' + PLUGIN_NAME + '-header').length) {
                this.headerHeight = parseInt(this.$element.find('.' + PLUGIN_NAME + '-header').innerHeight()) + 2;
            ...
            if (this.state == STATES.OPENED || this.state == STATES.OPENING) {
                if (this.options.iframe === true) {
                    if (windowHeight < (this.options.iframeHeight + this.headerHeight) || this.isFullscreen === true) {
						...
						//console.log(frameH);
						this.$element.find('.' + PLUGIN_NAME + '-iframe').css('height', frameH);
                    }
                }
